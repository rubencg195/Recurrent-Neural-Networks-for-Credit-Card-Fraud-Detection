...
def create_model(input_shape, output_dim, hidden_layers, hidden_layers_neurons, loss, optimizer, rnn_hidden_layers, rnn_hidden_layers_neurons, modelType="LSTM", dropout=True, dropout_rate=0.2  ):
    ...
    ________________________________ 
      ...
      output_dim:                {}
      \n""".format(
        rnn_hidden_layers, rnn_hidden_layers_neurons, hidden_layers, 
        hidden_layers_neurons, loss, optimizer, modelType, dropout, dropout_rate, 
    ...
    for i in range(rnn_hidden_layers):
      if(modelType == "LSTM"):
        model.add(LSTM(units=rnn_hidden_layers_neurons , input_shape=input_shape,  ))
      elif(modelType == "GRU"):
        model.add(GRU( units=rnn_hidden_layers_neurons  ,input_shape=input_shape ))
    for i in range(hidden_layers):
      model.add(Dense(hidden_layers_neurons, activation='relu'))
      if(dropout):
        model.add(Dropout(dropout_rate))
    model.add(Dense(output_dim, activation='sigmoid')) # model.add(Dense(1, activation='softmax'))
...
KEYS              : {}\n""".format( 
  ...
  history.best_estimator_.model.history.history,
  history.best_estimator_.model.history.history.keys()
))

# index_names = ["acc", "val_acc", "loss", "val_loss"]
index_names = list(history.best_estimator_.model.history.history.keys())
...
#SAVE MODEL LOCALLY
rnn.model.best_estimator_.model.save('rnn_best_model.h5')

history_dict = copy.copy(history.__dict__)
del history_dict['estimator']
del history_dict['best_estimator_']
history_dict
pickle.dump(history_dict, open( "rnn_best_model.history" , "wb" ) )
...
get_ipython().system('echo "Saving Files to ./{project_folder}"')
get_ipython().system('mkdir -p ./{project_folder}/images')
get_ipython().system('mkdir -p ./{project_folder}/data')
get_ipython().system('mkdir -p ./{project_folder}/model')


# In[198]:


get_ipython().system('cp *.data ./{project_folder}/data')
get_ipython().system('ls ./{project_folder}/data')


# In[199]:


get_ipython().system('cp *.png ./{project_folder}/images')
get_ipython().system('ls ./{project_folder}/images')


# In[200]:


get_ipython().system('cp *.h5 ./{project_folder}/model')
get_ipython().system('cp *.history ./{project_folder}/model')
get_ipython().system('ls ./{project_folder}/model')
